...
namespace DOTS.Terrain.Tests
{
    ...
    public class TreeChunkRenderSystemBoundsTests : SurfaceScatterRenderSystemContractTestsBase<TreeRenderConfig>
    {
        ...
        public void TryPrepareSubmissionFrame_DisabledInstancing_AutoEnablesInstancing()
        {
            ...
            try
            {
                var config = new TreeRenderConfig
                {
                    Mesh = mesh,
                    Material = material,
                    UniformScale = 1f,
                ...
                var prepared = TreeChunkRenderSystem.TryPrepareSubmissionFrame(config);
                Assert.IsTrue(prepared, "Tree render prep should succeed after auto-enabling material instancing.");
                Assert.IsTrue(material.enableInstancing, "Tree render prep should enable material instancing when disabled.");
            }
            finally
            {
        ...
        public void TryPrepareSubmissionFrame_MeshVariantsOnly_ConfigIsAccepted()
        {
            ...
            try
            {
                ...
                var prepared = TreeChunkRenderSystem.TryPrepareSubmissionFrame(config);
                Assert.IsTrue(prepared, "Variant-only config should be accepted for tree rendering.");
        ...
        public void TryPrepareSubmissionFrame_ErrorShaderMaterial_UsesRuntimeFallback()
        {
            ...
            try
            {
                ...
                var prepared = TreeChunkRenderSystem.TryPrepareSubmissionFrame(config);
                Assert.IsTrue(prepared, "Tree render prep should fall back to a runtime material when source shader is invalid.");
        ...
        protected override TreeRenderConfig CreateConfigWithLodVariants(
            Mesh[] meshVariants,
        ...
        {
            return TreeChunkRenderSystem.GetPendingMeshForTests(variantIndex, lodLevel);
        }
