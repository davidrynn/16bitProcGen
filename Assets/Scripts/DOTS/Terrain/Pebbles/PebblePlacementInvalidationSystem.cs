...
{
    /// <summary>
    /// Drops render-only pebble placement state for chunks queued for density rebuild.
    /// The next generation pass then rebuilds records from current density.
    /// </summary>
    [DisableAutoCreation]
    [BurstCompile]
    ...
    public partial struct PebblePlacementInvalidationSystem : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            foreach (var (_, entity) in SystemAPI.Query<RefRO<TerrainChunk>>()
                         ...
                         .WithEntityAccess())
            {
                SurfaceScatterLifecycleUtility.RemovePlacementStateIfPresent<PebblePlacementRecord, ChunkPebblePlacementTag>(
                    entity,
                    ref placementLookup,
                    ref tagLookup,
                    ref ecb);
