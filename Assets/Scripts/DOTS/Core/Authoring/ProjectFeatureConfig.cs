...
namespace DOTS.Core.Authoring
{
    ...
    public class ProjectFeatureConfig : ScriptableObject
    {
        ...
        [Tooltip("V13 descent VFX burn-off band (world Y): the flames fade from full at FadeStartY " +
                 ...
                 "sooner / higher up. Must keep FadeStartY > FadeEndY. FadeStartY is raised to " +
                 "SkyDropSpawnHeight at install time if it is set lower, so the fade always begins " +
                 "immediately on ignition — setting it below the spawn height has no effect. " +
                 "Tune FadeEndY to control how fast the burn-off completes.")]
