...
| R48 | Six test locations, five conventions (`DOTS/Test`, `DOTS/Tests`, `DOTS/TestHelpers`, `Player/Test`, `Player/Bootstrap/Tests`, `Assets/Tests/PlayMode`) | consolidate to two; scope AFTER this round's archives shrink the problem (A1 removes `Test/Archive`, A2 most of `DOTS/Debug`) | folder | **investigated 2026-07-03 (round 2)** — findings: 5 test asmdefs + 11 MonoBehaviours compiling into production `DOTS.Terrain` (→ A21/A22, most look dead); 7 namespace conventions. **Proposed target:** all NUnit tests under `DOTS/Tests/` with exactly two test asmdefs — `EditMode/` (existing 34-file asm) and `PlayMode/` (merge `DOTS.Terrain.Tests` + `Player.Tests` + `Smoke.PlayMode.Tests` + `DOTS.Player.Bootstrap.Tests`; the EditMode asm re-points its helper reference to the merged asm); manual-harness question resolved by the A22 verdict rather than a third location. **Approved** (owner 2026-07-03: "go with your recommendations") → batch 24 | |
...
| A16 | `DOTS/WFC/HybridWFCSystem.cs` | Never created by any production code; known gap per `Assets/Docs/Tickets/backlog.md` (ticket W1). | **keep — WFC paused, work resuming shortly** (owner 2026-07-02); bootstrap gap stays with the ticket | |
...
**Round-2 archive rows (from R48 investigation, 2026-07-03):**

| # | Path | Signal (why suspected dead) | Verdict | Batch |
|---|------|------------------------------|---------|-------|
| A21 | `Assets/testa.unity` | Scratch-named scene at Assets root; not in Build Settings; last touched 2026-06-09; only significance is being the sole attachment site of `QuickTerrainEntityCreator` | **archive** (owner 2026-07-03: "go with your recommendations") | 23 |
| A22 | `DOTS/Test/` (4 files: `DOTSDisabler`, `DungeonPrefabCreator`, `QuickTerrainEntityCreator`, `WFCSmokeHarness`) + `DOTS/TestHelpers/` (7 files: `AutoTestSetup`, `DOTSWorldSetup`, `GlobPhysicsTestSetup`, `HybridTestSetup`, `SettingsManager`, `VisualTestSceneSetup`, `WeatherTestSetup`) | **All 11 compile into the production `DOTS.Terrain` assembly** (no isolating asmdef — same smell as A2). GUID scene/prefab sweep: only `QuickTerrainEntityCreator` is attached anywhere, and only in `testa.unity` (A21). Code-reference sweep: zero references from any non-TestHelpers code, including the NUnit suites. `VisualTestSceneSetup` references A2-survivor `SimpleVisualDebugTest` (not vice versa). Caveats: `WFCSmokeHarness` may be wanted for resuming WFC work (owner call); these are editor-menu/manual harnesses, so "unreferenced" is a weaker deadness signal than for systems | **archive all 11** (owner 2026-07-03; WFC harness recoverable via git). **Amendments in-batch:** (1) `SimpleVisualDebugTest` cascades — its A2 keep-reason (referenced by `VisualTestSceneSetup`) dissolves with this batch; verified no other refs, not scene-attached; `DOTS/Debug/` folder now gone entirely. (2) Three `.md` docs living in `DOTS/Test/` (`Testing_Documentation`, `SeamlessTerrain_Solution`, `Test_Scripts_Notes`) document the deleted harnesses — archived to `Assets/Docs/Archives/ManualTestScripts_2026/` per doc rules (not code-deleted), indexed, CLAUDE.md refs updated | 23 |
...
| T16 | `Assets/.cursor/plans/game-production-plan-*.plan.md` (found post-T15, owner question) | Cursor AI plan (April 2026) inside a dot-folder Unity ignores; MASTER_PLAN called it the "primary task driver" but content-diff showed ~90% duplicated/stale vs MASTER_PLAN + TICKETS (references round-1-deleted systems); 3 residual nuggets: Phase 3-6 sketches, success metrics, one untracked UX item | **archive** (owner 2026-07-03) — done: archived to `Archives/RootLegacy_2026/GameProductionPlan_Cursor_2026-04.md`; MASTER_PLAN task-driver pointer → `TICKETS.md`; blocked-edit reticle UX → backlog ticket **E1** (post-MVP, owner call); repo-root `.cursor/rules/project-navigation.mdc` created so Cursor sessions land on MASTER_PLAN/TICKETS/INDEX (correct Cursor mechanism vs a plans/ stub, which Cursor only writes to) | 26 |
| T17 | `Assets/Art/` (found post-T15, owner question) | Empty scaffold: only two `.gitkeep`s (`Blender/`, `Textures/Palettes/`) from the Nov-2025 reorg; aspiration traced to the archived Cursor plan's Phase-4 layout; the convention that actually won is `Models/` + `Materials/` + repo-root `BlenderSource/`; zero doc/code references | **archive/delete** (owner 2026-07-03: "delete it as T17") — done | 27 |

**Resources-folder audit (owner-requested, 2026-07-03).** Method: every `Resources/` file cross-checked against literal + const-path `Resources.Load` calls, `LoadAll` sweeps, `Shader.Find`, and GUID references. Verified-live and untouched: 8 compute shaders + GrassBlades + ProceduralGradientSky shaders, TerrainGenerationSettings, Terrain/TerrainChunkRenderSettings, GrassBladeMesh, Dungeon/Relic rulesets (`LoadAll("")`), CloudbreakSkyPreset + DefaultBiomeSkyMapping (scene-assigned).

| # | Path | Evidence | Verdict | Batch |
|---|------|----------|---------|-------|
| U1 | `Resources/WeatherEffects/` (9 prefabs + 3 audio) | Zero code loads, zero GUID refs — legacy GameObject weather system; current weather = compute shaders. Force-shipped in every build for nothing | **archive** (owner 2026-07-03: direction undecided — one `git revert` restores). Cascade: `Models/TriArm2.0.fbx` (T14) is now zero-ref; rides the weather decision | 28 |
| U2 | `Resources/Shaders/BasicTest.compute` | Zero refs; test shader | **archive** (owner 2026-07-03) | 28 |
| U3 | `Resources/Terrain/TerrainChunkGrassMaterial.mat` | Zero refs; grass material comes from the render system's own resource path | **archive** (owner 2026-07-03) | 28 |
| U4 | `Resources/Sky/DefaultSkyPreset.asset` | Zero refs since V6 switched the scene to CloudbreakSkyPreset; regenerable via `Tools/Sky/Create Default Sky Preset` | **archive** (owner 2026-07-03) | 28 |
| U5 | `Resources/Materials/ProceduralGradientSky.mat` | Scene-GUID-referenced only (skybox slot) — Resources placement unnecessary; the *shader* stays in `Resources/Shaders/` for `Shader.Find` | **move to `Assets/Materials/`** (owner 2026-07-03: "put it where it should be"); empty `Resources/Materials/` removed | 28 |
| U6 | `Assets/Materials/` — 10 zero-ref files: Blue.mat, Purple.mat, Unlit green.mat, Unlit Test Red.mat, TreeVertexColorURP.mat, M_YFGM_Grass01.mat, T_Grass.mat + T_Grass_Field_C.PNG, NewUnlitShader.shader, New Render Texture.renderTexture | Owner-requested materials audit (scripted GUID sweep across BOTH worktrees incl. `fix/v7-fallthrough`, + code-name greps). Zero references anywhere; M_YFGM/T_Grass carry third-party-pack names (vendor-asset rule). 8 live files verified and kept (Generic_Grass = the terrain material via TerrainChunkRenderSettings; TerrainMat/Unlit gray/ProceduralGradientSky in build scene; Brown/DungeonTestMaterial via MacroTile prefabs; ColorQuantize→QuantizationShader via LODSettings) | **archive** (owner 2026-07-03: "delete all 13") | 29 |
| U7 | `Assets/Materials/Rain.mat` + `rain_streak.png` + `SandStorm.mat` | Weather-cascade: only references were the U1 prefabs deleted in batch 28 | **archive** (owner 2026-07-03 — same revert story as U1) | 29 |
| U8 | `Assets/TestReports_SPEC_Implementation/` | Completely empty — no files, no tracked contents, only a folder `.meta` | **archive** (owner 2026-07-03: "pretty sure you can delete") | 30 |
| U9 | `Assets/SubScenes/` | `.gitkeep`-only scaffold (same pattern as T17 `Art/`). SubScenes ARE a DOTS feature (baking workflow) but this project committed to runtime spawning (CLAUDE.md rule 1); folder never used. Recreate if baking is ever adopted | **archive** (owner 2026-07-03) | 30 |
| U10 | `Assets/Settings/` (7 URP assets) | **KEEP — verified live, do not re-audit:** `GraphicsSettings`→`PC_RPAsset`→`PC_Renderer`; `QualitySettings`→`Mobile_RPAsset`→`Mobile_Renderer`; URP GlobalSettings→`DefaultVolumeProfile`; `SampleSceneProfile` survived its deleted namesake scene — it is the default volume profile slot of BOTH RP assets | **keep** (verified 2026-07-03) | 30 |
...
| D9 | ~16 unindexed live docs incl. `TICKETS.md`, `AI/VISTA_GROUND_PLANE_FOG_INVESTIGATION.md`, `AI/GRASS_ECS_SPEC.md`, `AI/SKYBOXPLAN.md`, `AI/TERRAIN_VOXEL_CHUNK_EDIT_SPEC.md`, biome-noise trio, `MVP Movement/` 2 specs | index drift (one-directional: missing entries only) | **add to `DOCUMENT_INDEX.md`** — done; also indexed `TERRAIN_MVP_PRIORITY_NOTE.md`, `DOCUMENTATION_CHANGELOG.md`, and the D11 external-reference folder | 20 |
| D10 | Folders `AI/STRUCTURE PLACEMENT/` and `MVP Movement/` | spaces in folder names force %20 links | **rename `STRUCTURE_PLACEMENT/`, `MVP_Movement/` + fix links** — done; all %20/space refs fixed across docs (none found in CLAUDE.md/.cursor/.github) | 20 |
| D11 | `Assets/Docs/ai-patterns-other-project/` (5 files) | reference pack from an unrelated project (SmolbeanPlanet3D); false-positive magnet in searches | **relocate to `Assets/Docs/Reference/External/SmolbeanPlanet3D/`** — done; single "External Reference" index line, contents deliberately unindexed | 20 |
| D12 | ~15 core docs with no Status/Last-Updated header (incl. `AI/TERRAIN_ECS_NEXT_STEPS_SPEC.md`, `PROJECT_STRUCTURE_DOTS.md`, `WFC/MAP_WFC.md`) | fails SYSTEM_SPEC metadata rule; `PROJECT_STRUCTURE_DOTS.md` also stale vs reality | **backfill metadata; rewrite PROJECT_STRUCTURE_DOTS after code moves land** — metadata backfilled on 16 docs (Last-Updated = last git commit date); PROJECT_STRUCTURE_DOTS marked PARTIALLY STALE, content rewrite deferred to batch 21 | 20 |
| D13 | `ArtAndDOTS_Pipeline.md` (root) | agent asset-pipeline guide sitting at root | **move to `AI/`** — done; 4 referencing docs updated | 20 |

**Round-2 doc rows (found 2026-07-03, post-round-1 review):**

| # | Doc | Issue (stale status / duplicate / misplaced / unindexed) | Verdict | Batch |
|---|-----|-----------------------------------------------------------|---------|-------|
| D14 | Repo-root `Docs/Tests/` (2 files: `Codex_PlayModeSmokeTestPlan.md`, `SmokeSceneSetup.md`, Dec 2025) | Outside `Assets/` entirely, so the round-1 audit (scoped to `Assets/Docs/`) never saw it; unindexed, not imported by Unity. Content is **current**, not stale — documents the live `Smoke_BasicPlayable.unity` setup | **approved:** move into `Assets/Docs/` (destination folder per D15), index both, delete the root `Docs/` folder — done (→ `Testing/`) | 22 |
| D15 | Doc folder taxonomy: `AI/` vs `Assets/Docs/` root is undefined | `DOCUMENTATION_SYSTEM_SPEC.md` defines statuses, suffixes, indexing, and archives but never says what `AI/` *means*. De facto it's "docs written in AI sessions" — categorization by author, not topic — and it has grown into a ~28-file junk drawer spanning terrain, player, skybox, multiplayer, persistence. Since discovery is index-driven (spec §3/§8), the folder name carries no retrieval value for AI or humans | **approved (owner, 2026-07-03): Option A — feature/area folders.** Codified as `DOCUMENTATION_SYSTEM_SPEC.md` §6.4; 53 docs re-homed; `AI/` dissolved. **Amendment:** `GRASS_ECS_SPEC.md` was absent from the approved mapping (oversight) — filed under `Terrain/Scatter/` to empty `AI/` | 22 |
| D16 | Feature siblings split across folders: `DOTS_Terrain_LOD_SPEC.md` at root while `DOTS_Terrain_LOD_Implementation_Checklist.md` is in `AI/`; `WFC_Dungeon_Test_Plan.md` at root while WFC docs live in `WFC/` | same-topic docs should be co-located (spec §5) | **approved** — re-homed per Option A (LOD pair → `Terrain/`, dungeon test plan → `WFC/`) | 22 |

> **D15 options (recorded 2026-07-03; owner picked A same day):**
> - **A — Feature/area folders (CHOSEN):** folders mirror the architecture (`Terrain/`, `Player/`, `Rendering/`, `WFC/`, `Structures/`, `Testing/`, plus `Process/` for meta-docs like this plan and the art pipeline); doc *type* stays in the filename suffix (`_PLAN`/`_SPEC`/`_CHECKLIST`/`_REPORT`); root reserved for project-wide living docs (`DOCUMENT_INDEX`, `TICKETS`, `KNOWN_ISSUES`, `MASTER_PLAN`, `PROJECT_STRUCTURE_DOTS`, `DOCUMENTATION_SYSTEM_SPEC`); `AI/` dissolves. Half-adopted already (`WFC/`, `Biomes/`, `STRUCTURE_PLACEMENT/`, `MVP_Movement/`, `TerrainHeightMaps/`).
> - **B — Doc-type folders (`Specs/`, `Plans/`, `Reports/`):** rejected by recommendation — splits one feature's siblings across folders and duplicates what the filename suffix already encodes.
> - **C — Keep `AI/`, just define it:** lowest churn, but preserves author-based categorization that answers no question a reader actually has.
> - Initiative-named folders (e.g. `MVP*/`) age poorly — round 1 already had to rename `mvp/`→`Biomes/` (D7); "MVP" is lifecycle info that belongs in Status metadata, not the folder tree.
...
- **2026-07-03** — **Round 2 execution completed** (batches 22–24 on `chore/cleanup-round-2`, all gated green; PlayMode verified identical to baseline after the test consolidation). Owner verdicts mid-round: D15 Option A, A21/A22 archive-all, R48 two-assembly target — all recorded in their rows. New hazard for the checklist: renaming a namespace off the `DOTS.Terrain.*` ancestor chain silently removes implicit type resolution (fix: explicit usings / namespace aliases; hit in batch 24). Round-2 leftovers: none in scope — §6.7 S1–S16 remain the spun-off backlog; S13 does not exist (numbering gap, not a lost row). Round 2: 253 files, net −3.5k lines; rounds 1+2 combined: 676 files, net −16.4k lines (a9ec90e..HEAD, 32 commits + merge).
- **2026-07-03** — **D15 decided (owner): Option A — feature/area doc folders.** Folders mirror the architecture (`Terrain/`, `Player/`, `Rendering/`, `WFC/`, `Structures/`, `Testing/`, `Process/` for meta-docs); doc type stays in filename suffixes; docs root reserved for project-wide living docs; `AI/` dissolves; initiative-named folders avoided (lifecycle belongs in Status metadata — `mvp/`→`Biomes/` precedent, D7). Rationale: "AI" categorized by author, not topic, and index-driven discovery (spec §3/§8) makes the folder name worthless for retrieval; type folders would split feature siblings. To execute as round 2's first batch: codify rule in `DOCUMENTATION_SYSTEM_SPEC.md`, then D14+D16 moves with full link/index repair (~35–40 files).
- **2026-07-03** — **Round-2 doc seeds opened** (post-round-1 review with owner). Owner spotted two structural oddities the round missed: a repo-root `Docs/Tests/` folder outside the audit's `Assets/Docs/` scope (D14), and no defined rule for what belongs in `AI/` vs the docs root (D15/D16). Taxonomy decision is the owner's; options recorded under §6.3. Also added §6.8 merge notes after worktree playtest.
- **2026-07-02** — **Round 1 execution completed** (21 batches, all committed on `chore/codebase-cleanup`, EditMode green after every batch; PlayMode verified regression-free against pre-cleanup baseline). Mid-execution amendments, each recorded in §6.6: A19→quarantine; enums BiomeType/TerrainType/NoiseType kept (live consumers); SceneDiagnostics kept minus legacy check; GlobRemovalType extracted (live glob physics); R43 physical move blocked by asmdef cycle (namespace fix sufficed); R63 partial (preprocessor-guarded type stayed); C5 survivor = ChunkProcessor (ISystem+DebugSettings) renamed TerrainDataValidationSystem — validator warnings now DebugSettings-gated. Recurring hazards worth remembering next round: UnityEngine.TerrainData ambiguity, DOTS.Terrain.Debug shadows UnityEngine.Debug inside DOTS.Terrain.*, asmdef references are non-transitive and non-cyclic (check before adding usings), Unity batchmode can't run WaitForEndOfFrame tests.
...
| 19 | 2026-07-02 | D1–D8 | n/a (docs only) | 54fbeb5 | 21 docs archived into 8 new `Archives/*_2026` subfolders + `DebugTraces/` moved whole; every archived doc got an ARCHIVED (2026-07-02) status line. `mvp/` renamed to `Biomes/` (1 file left). LOD cluster content-diffed: unique namespace/edit-authoritative-promotion/neighbor-clamp/debug-flag notes merged into root `DOTS_Terrain_LOD_SPEC.md` before archiving the 2 AI/ originals. `StructureReview.md` open finding folded into §6.7 as S16. `DOCUMENT_INDEX.md` updated for every moved doc; `DOCUMENTATION_CHANGELOG.md` given a new 2026-07-02 entry |
| 20 | 2026-07-02 | D9–D13 | 210/210 (meta validation) | c39523a | D10: `STRUCTURE PLACEMENT`→`STRUCTURE_PLACEMENT`, `MVP Movement`→`MVP_Movement` (git mv + metas), all %20/space links fixed, no refs outside Assets/Docs. D11: `ai-patterns-other-project/`→`Reference/External/SmolbeanPlanet3D/`. D13: `ArtAndDOTS_Pipeline.md`→`AI/` (4 refs updated). D9: 13 index entries added (incl. External Reference section). D12: Status/Last-Updated backfilled on 16 docs from git commit dates; PROJECT_STRUCTURE_DOTS content rewrite deferred to batch 21 |
| 21 | 2026-07-02 | R49, D12(rewrite), A20 | 210/210 | *(this commit)* | CLAUDE.md: namespace table rewritten to post-cleanup reality (R45/R46 exceptions documented, Legacy marked do-not-extend), renamed systems fixed in Dual Pipelines / Key Systems / WFC flow / Extending Systems, `using DOTS.Core` in logging example. PROJECT_STRUCTURE_DOTS.md rewritten (real folder tree, honest asmdef state, current system lists). DEPRECATED_FILES.md ledger archived per A20 — this plan is now the sole tracker. **ROUND 1 COMPLETE** |
...
| 26 | 2026-07-03 | T16 | EditMode 210/210 | *(this commit)* | Cursor plan archived + `.cursor/` relocated to repo root as a rules pointer; MASTER_PLAN repaired: task-driver → TICKETS.md, stale refs fixed (CameraFollowSystem/PlayerCameraSystem → CameraEffectResolverSystem, Testing_Documentation → Tests/README, HybridTerrainGeneration → LegacyHeightmap rename, DOTS/World → Impostors); TICKETS backlog + E1 |
| 27 | 2026-07-03 | T17 | EditMode 210/210 | *(this commit)* | Empty `Assets/Art/` scaffold deleted (2 `.gitkeep`s + 4 folder metas); real art convention (`Models/`, `Materials/`, `BlenderSource/`) unchanged |
| 28 | 2026-07-03 | U1–U5, S17 | EditMode 210/210; PlayMode 96 = baseline exactly (86/9/1) | *(this commit)* | Resources audit executed: WeatherEffects folder (12 files) + BasicTest.compute + TerrainChunkGrassMaterial + DefaultSkyPreset archived; ProceduralGradientSky.mat → `Assets/Materials/`; S17 grass-settings load-path bug fixed in code (verified no behavioral delta today). TriArm2.0.fbx flagged zero-ref (rides weather decision) |
| 29 | 2026-07-03 | U6, U7 | EditMode 210/210 | *(this commit)* | 13 dead materials/textures/shaders deleted from `Assets/Materials/` (10 zero-ref + 3 weather-cascade); 8 verified-live files remain; audit method: scripted GUID sweep over both worktrees (script beat Haiku on cost per plan §2 rules); S18 build-fallback observation logged |
| 30 | 2026-07-03 | U8–U10 | EditMode 210/210 | *(this commit)* | Empty `TestReports_SPEC_Implementation/` + `SubScenes/` scaffold deleted; `Settings/` verified fully live (URP pipeline config) and kept. Owner also cleared `Prefabs/` + `ScriptableObjects/` by inspection — every `Assets/` top-level folder has now been reviewed in rounds 1–3 |
...
| S3 | `HybridWFCSystem` / `DotsSystemBootstrap` | Dungeon pipeline silently broken: system never created anywhere though `DungeonRenderingSystem` orders after it. Already tracked at `Assets/Docs/Tickets/backlog.md` (ticket W1) — wire under `EnableDungeonSystem` or decide to shelve WFC (A16) | 2, 3 | open (ticket exists) |
...
| S17 | `GrassSystemSettings.cs` | **Bug (found in U-audit):** `Resources.Load("GrassSystemSettings")` but the asset lives at `Resources/Terrain/` — load silently returned null; all grass systems ran on `GrassDefaults` instead of the asset. Fixed: const → `"Terrain/GrassSystemSettings"` + why-comment. Safe: asset values currently mirror defaults (`BaseMaterial` unassigned but unused by consumers; both consumers null-tolerant) — behavior identical until the asset is tuned, which now works | 3 | **fixed in batch 28** (owner-approved 2026-07-03) |
| S18 | `SurfaceScatterRenderCore` fallback path | Build-correctness risk (found in U6 audit): runtime fallback uses `Shader.Find("DOTS/VertexColorUnlitClip")`, but that shader lives outside `Resources/` with zero material references — works in-editor, returns null in a player build (degrades to error shader). Fix when builds matter: move shader to `Resources/` or add to Always Included Shaders or reference it from a shipped material | 3 | open |
