...
- Neighboring cell vertices are stitched into quads (split into triangles) across XY, XZ, and YZ faces when adjacent cell signs differ, producing a watertight mesh.
...
### Sampling
- **TerrainChunkDensitySamplingSystem** (`ISystem`): Requires SDF settings; iterates chunk entities, samples SDF into a temporary array via `TerrainChunkDensitySamplingJob`, and stores results in a blob component on each chunk. Copies buffered `SDFEdit` entries into temp memory before sampling.

### Meshing
- **TerrainChunkMeshBuildSystem** (`ISystem`): Runs in the Simulation group for chunks tagged `TerrainChunkNeedsMeshBuild`. Invokes `TerrainChunkMeshBuilder.BuildMeshBlob`, which runs `SurfaceNetsJob` to convert densities to mesh blobs, attaches `TerrainChunkMeshData`, adds render-upload tag, and removes the mesh-build tag.
...
### Assets/Scripts/Terrain/SDF/SDFTerrainFieldSettings.cs
...
### Assets/Scripts/Terrain/SDF/Chunks/TerrainChunk*.cs
...
### Assets/Scripts/Terrain/SDF/Systems/TerrainChunkDensitySamplingSystem.cs
- Requires `SDFTerrainFieldSettings`; copies any `SDFEdit` buffer to a temp array.
- For each chunk, schedules/runs `TerrainChunkDensitySamplingJob` to fill a temp density array across resolution using chunk origin and voxel size.
- Converts sampled densities into a persistent blob (`TerrainChunkDensity`) on the chunk, disposing previous blob if present.

### Assets/Scripts/Terrain/Meshing/TerrainChunkMeshBuilder.cs
- Converts density blob to a mesh blob: copies densities into a temp `NativeArray`, allocates working arrays (cell signs and vertex indices), and runs `SurfaceNetsJob`.
...
- Implements the Surface Nets job:
  ...
  - Builds triangles on XY/XZ/YZ faces when adjacent cell signs differ; outputs triangles with consistent winding based on sign sum.
