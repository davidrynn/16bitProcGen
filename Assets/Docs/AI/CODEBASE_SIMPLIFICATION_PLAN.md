...
**Status:** ROUND 1 COMPLETE (2026-07-02) — 21 batches executed and green; round-2 seeds: R48/C9 test consolidation, D14–D16 doc taxonomy (D15 awaits owner decision), §6.7 suggestions S1–S16
...
| D15 | Doc folder taxonomy: `AI/` vs `Assets/Docs/` root is undefined | `DOCUMENTATION_SYSTEM_SPEC.md` defines statuses, suffixes, indexing, and archives but never says what `AI/` *means*. De facto it's "docs written in AI sessions" — categorization by author, not topic — and it has grown into a ~28-file junk drawer spanning terrain, player, skybox, multiplayer, persistence. Since discovery is index-driven (spec §3/§8), the folder name carries no retrieval value for AI or humans | **needs-decision (owner)** — options recorded below; once decided, codify in `DOCUMENTATION_SYSTEM_SPEC.md` as a Folder Taxonomy section and execute moves + link/index fixes with batch-19/20 mechanics | |
| D16 | Feature siblings split across folders: `DOTS_Terrain_LOD_SPEC.md` at root while `DOTS_Terrain_LOD_Implementation_Checklist.md` is in `AI/`; `WFC_Dungeon_Test_Plan.md` at root while WFC docs live in `WFC/` | same-topic docs should be co-located (spec §5) | **rides D15** — re-home per whichever taxonomy is chosen | |

> **D15 options (recorded 2026-07-03; owner to pick):**
> - **A — Feature/area folders (AI recommendation):** folders mirror the architecture (`Terrain/`, `Player/`, `Rendering/`, `WFC/`, `Structures/`, `Testing/`, plus `Process/` for meta-docs like this plan and the art pipeline); doc *type* stays in the filename suffix (`_PLAN`/`_SPEC`/`_CHECKLIST`/`_REPORT`); root reserved for project-wide living docs (`DOCUMENT_INDEX`, `TICKETS`, `KNOWN_ISSUES`, `MASTER_PLAN`, `PROJECT_STRUCTURE_DOTS`, `DOCUMENTATION_SYSTEM_SPEC`); `AI/` dissolves. Half-adopted already (`WFC/`, `Biomes/`, `STRUCTURE_PLACEMENT/`, `MVP_Movement/`, `TerrainHeightMaps/`).
