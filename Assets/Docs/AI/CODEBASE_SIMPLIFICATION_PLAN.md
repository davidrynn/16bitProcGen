...
**Status:** ROUND 1 COMPLETE (2026-07-02) — 21 batches executed and green; round-2 seeds: R48/C9 test consolidation, D14–D16 doc taxonomy (Option A approved 2026-07-03, not yet executed), §6.7 suggestions S1–S16
...
| # | Doc | Issue (stale status / duplicate / misplaced / unindexed) | Verdict | Batch |
|---|-----|-----------------------------------------------------------|---------|-------|
| D14 | Repo-root `Docs/Tests/` (2 files: `Codex_PlayModeSmokeTestPlan.md`, `SmokeSceneSetup.md`, Dec 2025) | Outside `Assets/` entirely, so the round-1 audit (scoped to `Assets/Docs/`) never saw it; unindexed, not imported by Unity. Content is **current**, not stale — documents the live `Smoke_BasicPlayable.unity` setup | **approved:** move into `Assets/Docs/` (destination folder per D15), index both, delete the root `Docs/` folder | |
| D15 | Doc folder taxonomy: `AI/` vs `Assets/Docs/` root is undefined | `DOCUMENTATION_SYSTEM_SPEC.md` defines statuses, suffixes, indexing, and archives but never says what `AI/` *means*. De facto it's "docs written in AI sessions" — categorization by author, not topic — and it has grown into a ~28-file junk drawer spanning terrain, player, skybox, multiplayer, persistence. Since discovery is index-driven (spec §3/§8), the folder name carries no retrieval value for AI or humans | **approved (owner, 2026-07-03): Option A — feature/area folders.** Codify as a Folder Taxonomy section in `DOCUMENTATION_SYSTEM_SPEC.md`, then execute moves + link/index fixes with batch-19/20 mechanics. First execution batch of round 2 | |
| D16 | Feature siblings split across folders: `DOTS_Terrain_LOD_SPEC.md` at root while `DOTS_Terrain_LOD_Implementation_Checklist.md` is in `AI/`; `WFC_Dungeon_Test_Plan.md` at root while WFC docs live in `WFC/` | same-topic docs should be co-located (spec §5) | **approved** — re-home per Option A (rides the D15 batch) | |
