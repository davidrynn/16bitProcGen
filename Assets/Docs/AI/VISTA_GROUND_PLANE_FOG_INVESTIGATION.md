...
> "unbuilt" assumption in TICKETS.md. Promote decisions into the source specs once confirmed.
...
- **Live and enabled:** `EnableGroundPlaneImpostor: 1`; the `GroundPlaneImpostorBootstrap` GameObject
  is in `Basic Terrain Scene.unity` with `_enabled: 1`. (V1 is **built**, contradicting TICKETS.md's
  "mostly unbuilt".)
