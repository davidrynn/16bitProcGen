...
| V14 | [ ] | Meteor-interior loading shell — diegetic initial load: full-screen meteor interior over the V7 readiness gate, breaks open on real gate release (binary + min-hold, no fake progress). **Built 2026-07-18** — min-hold moved into the gate itself (gravity + break-open one beat by construction), overlay polls gate-component removal (no new bridge), fully procedural visuals (packed texture + dissolve shader), EditMode gate-timing tests green. Remaining: owner eyeball of the arrival beat in play (opened 2026-07-08; `Rendering/METEOR_ARRIVAL_SEQUENCE_SPEC.md` §12) |
