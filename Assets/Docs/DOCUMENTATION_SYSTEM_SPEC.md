...
2. **The `Assets/Docs/` root is reserved for project-wide living docs** — the index, changelog, this spec, `KNOWN_ISSUES.md`, `MASTER_PLAN.md`, `PROJECT_NOTES.md`, `PROJECT_STRUCTURE_DOTS.md`. If a doc is about one feature, it goes in that feature's folder. (`TICKETS.md` moved to its own `Tickets/` folder on 2026-07-05 — it outgrew a single file per the §5 subfolder rule; `Tickets/` holds the project-wide ticket tracker, not one feature area, so it sits directly under `Assets/Docs/` rather than under a feature folder.)
